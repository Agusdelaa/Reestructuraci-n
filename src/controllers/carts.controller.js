...
export default class CartsController {
    ...
    async updateProductQuantity(req, res) {
        try {
            ...
            const productIndex = cart.products.findIndex(product => product.product._id.toString() === pid)
            if (productIndex === -1) {
                return res.sendUserError(`No se encontro el producto de id ${pid} en el carrito de id ${cid}`)
            }
            cart.products[productIndex].quantity = quantity
            const payload = await CartsServices.getInstance().updateCart(cid, cart.products)
            res.sendSuccessPayload(payload)
    ...
    async removeProduct(req, res) {
        try {
            ...
            const productIndex = cart.products.findIndex(product => product.product._id.toString() === pid)
            if (productIndex === -1) {
                return res.sendUserError(`No se encontro el producto de id ${pid} en el carrito con de ${cid}`)
            }
